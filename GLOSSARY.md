...
| 术语 | 定义 |
|---|---|
| EAP | Environment Awareness Protocol。用于环境感知契约、规则评估和注意力事件交付的通用协议。 |
...
| Broker | 转发或聚合 EAP 事件、Contract、Capability 的中间组件。Broker 不改变 Rule 语义。 |
| Environment | 感知发生的边界。每个 Environment 使用 EAP URI 标识。 |
| Subject | 被观察对象。每个 Subject 使用 EAP URI 标识。 |
...
| Condition | Rule 中的布尔表达式节点。EAP Condition 不执行任意代码。 |
...
| EAP URI | `eap://authority/type/id` 格式的标识符。 |
| Privacy Policy | Contract 中的用途、最小化、脱敏、保留和审计策略。 |
| Privacy Report | Attention Event 中说明实际包含与脱敏字段的报告。 |
| MCP Binding | 将 EAP Lifecycle API 暴露为 MCP tools/resources/prompts 的 agent-to-tool 绑定。 |
| A2A Binding | 将 EAP Provider 表示为 A2A agent，并以 task、message、part、artifact 传递 EAP 对象的 agent-to-agent 绑定。 |
