...
Domain Profile 是特定场景的默认 Rule、Delivery、Privacy 和参数模板。Profile 不改变 ExAP Core 语义。Profile JSON MUST 使用 `schemas/exap-profile.schema.json` 校验通过。
...
| 文件 | Profile ID | 场景 |
|---|---|---|
| `process-monitoring.exap.json` | `profile:process-monitoring` | 长时间进程和资源监控。 |
| `deep-learning-training.exap.json` | `profile:deep-learning-training` | 深度学习训练任务、GPU、日志、checkpoint。 |
| `email-priority.exap.json` | `profile:email-priority` | 重要邮件提醒。 |
| `calendar-focus.exap.json` | `profile:calendar-focus` | 重要日历事件提醒。 |
| `file-watch.exap.json` | `profile:file-watch` | 文件生成、修改和稳定性。 |
| `iot-safety.exap.json` | `profile:iot-safety` | IoT 传感器和安全事件。 |
| `ci-cd-monitor.exap.json` | `profile:ci-cd-monitor` | 构建、测试、部署流水线。 |

## 5. Profile 应用流程

1. Consumer 在 `exap.discover` 返回的 `profiles` 中选择 profile。
